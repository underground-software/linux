...
static int c_show(struct seq_file *m, void *v)
{
	...
	cpuid = is_smp() ? per_cpu(cpu_data, n).cpuid : read_cpuid_id();
	seq_printf(m, "model name\t: %s rev %d (%s)\n",
		   cpu_name, cpuid & 15, elf_platform);

#if defined(CONFIG_SMP)
	seq_printf(m, "BogoMIPS\t: %lu.%02lu\n",
		   per_cpu(cpu_data, n).loops_per_jiffy / (500000UL/HZ),
		   (per_cpu(cpu_data, n).loops_per_jiffy / (5000UL/HZ)) % 100);
#else
	seq_printf(m, "BogoMIPS\t: %lu.%02lu\n",
	...
	if (cpumask_last(cpu_online_mask) == n) {
